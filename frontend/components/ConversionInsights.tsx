            ...
            <div className="mx-auto max-w-[1720px] w-full">
                ...
                <div className="mt-16 grid grid-cols-1 gap-8 md:grid-cols-3 md:gap-10 lg:gap-12 w-full">
                    {(data.cards || []).map((card, index) => {
                        ...
                        const descMaxWidth = index === 0 ? "max-w-[315px]" : "max-w-[335px]";
                        ...
                        const getContainerPadding = (idx: number) => {
                            ...
                            return "p-[24px_24px_0] min-[460px]:p-[26px_28px_0] md:px-8 md:pt-8 md:pb-0";
                        };

                        return (
                            <article
                                key={card.id}
                                className={`overflow-hidden rounded-[12px] md:rounded-[24px] ${bgColor} ${textColor} flex flex-col h-full min-h-[330px] min-[460px]:min-h-[350px] md:min-h-[415px] lg:min-h-[425px] justify-start w-full`}
                            >
                                {imageUrl && (
                                ...
                                <div className="flex flex-col justify-start flex-grow p-[18px_24px_24px] min-[460px]:p-[20px_28px_26px] md:px-8 md:pt-5 md:pb-9">
                                    <h3 className={`font-nohemi font-normal whitespace-pre-line text-pretty ${textColor} text-[clamp(20px,1.8vw,26px)] leading-[1.2] tracking-[-0.02em] mb-4 md:mb-5 min-h-[48px] md:min-h-[64px] max-w-[340px] flex items-start`}>
                                        {card.title}
                                    </h3>
                                    ...
                                    {cardMobileDesc ? (
                                        <>
                                            ...
                                            <p className={`hidden md:block font-satoshi text-[clamp(12px,0.95vw,13px)] leading-[1.6] whitespace-pre-line text-pretty ${descMaxWidth} ${descriptionWeight} ${descriptionColor}`}>
                                                {card.description}
                                            </p>
                                        </>
                                    ) : (
